...
st.set_page_config(page_title="Godrej Capital — Prime LAP Risk Classifier", layout="wide", page_icon="■")
...
with tab1:
    ...
    with col2:
        ...
        loan_ask_lakhs = st.number_input("Loan Ask (₹ Lakhs)", 10.0, 1500.0, 120.0, step=10.0)
        revenue_lakhs = st.number_input("Revenue (₹ Lakhs)", 0.0, 5000.0, 350.0, step=10.0)
        gst_seasonality_cv = st.number_input("GST Seasonality CV", 0.0, 1.0, 0.15, step=0.01,
                                              help="0 = stable quarterly sales, >0.3 = volatile")
        net_profit_margin_pct = st.number_input("Net Profit Margin %", -20.0, 50.0, 15.0, step=0.5)
        cash_profit_lakhs = st.number_input("Cash Profit (₹ Lakhs)", 0.0, 500.0, 60.0, step=5.0)
    ...
    with col3:
        ...
        avg_bank_balance_lakhs = st.number_input("Avg Bank Balance (₹ Lakhs)", 0.0, 500.0, 15.0, step=1.0)
    ...
    if st.button("Classify Risk", type="primary", use_container_width=True):
        ...
        with r1:
            ...
            <div style="text-align:center; padding:20px; border:3px solid {color}; border-radius:12px; background:{color}11">
                ...
                <div style="font-size:12px; color:#888; margin-top:6px">Confidence: {pred_proba[pred_idx]*100:.1f}%</div>
            </div>
            """, unsafe_allow_html=True)

            st.markdown("**Derived Ratios**")
            st.metric("FOIR", f"{foir:.3f}", delta="High" if foir > 0.5 else "Normal", delta_color="inverse")
            st.metric("Cash Profit / Loan", f"{cash_profit_to_loan:.3f}", delta="Low" if cash_profit_to_loan < 0.3 else "Adequate")
        ...
        with r2:
            ...
            try:
                ...
                # shap_values is array of shape (1, 15, 3) or list of 3 arrays
...
with tab2:
    ...
    st.markdown(f"Columns: {len(df.columns)} · Rows: {len(df):,} · ~12% intentionally mislabeled · ~12% rows with missing values")
